...
CreateThread(function() Config.LoadPlugin("vehreg", function(pluginConfig)
    if pluginConfig.enabled then
        ...
        RegisterNetEvent(GetCurrentResourceName() .. "::registerVeh", function(primary, plate, class, realName)
	        ...
	        local player = source
            local first = players[player].firstName;
            local last = players[player].lastName;
            local dob = players[player].dob;
            local sex = nil;
            local mi = nil;
            local age = nil;
            local aka = nil;
            ...
            exports["sonorancad"]:performApiRequest({{
                ...
                if err == 404 then
                    TriggerClientEvent(
                        'chat:addMessage', source,
                        {
                            color = {255, 0, 0},
                            multiline = true,
                            args = {'[CAD - ERROR] ', pluginConfig.language.noApiId}
                        }
                    )
                    return;
                end
                ...
                if string.find(res, 'taken') ~= nil then
                    TriggerClientEvent(
                        'chat:addMessage', source,
                        {
                            color = {255, 0, 0},
                            multiline = true,
                            args = {'[CAD - ERROR] ', pluginConfig.language.plateAlrRegisted}
                        }
                    )
                else
                    local placeHolders = {
                    ...
                    TriggerClientEvent(
                        'chat:addMessage', source,
                        {
                            color = {0,255,0},
                            multiline = true,
                            args = {'[CAD - SUCCESS] ', placeholderReplace(pluginConfig.language.successReg, placeHolders)}
                        }
                    )
                end
            end)
